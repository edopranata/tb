...
class IncomeReport extends Component
{
    ...
    $reports = Sell::query()
        ->select('sells.invoice_number', DB::raw('SUM(sell_details.total) As sell_price '), DB::raw('SUM(sell_details.buying_price * sell_details.product_price_quantity) As buying_price'))
        ->leftJoin('sell_details', 'sells.id', '=', 'sell_details.sell_id')
